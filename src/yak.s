...
supf1: lea s_flip1,a1
	bra ccdraw

supf2: lea s_flip2,a1
	bra ccdraw

draw_blueflip: lea blueflipper,a1
	bra ccdraw
...
dr_beast3: lea hornm3,a1
	bra drawsolidxy

dr_beast2: lea hornm2,a1
	bra drawsolidxy

draw_beast: move #0,d6
	move 46(a6),d7
	and #3,d7
...
beastybits: dc.l hornm1,hornm2,hornm3,hornm3
...
cntdne:	ext.l d4		;get x-centre
	move.l #9,d5
	bra drawsolidxy
draw_sflipper: lea s_flipper,a1
	bra drawsolidxy


draw_mirr: lea mirr,a1
	bra drawsolidxy


draw_spshot: lea pshot,a1
	bra drawsolidxy
...
draw_sfliptank:	lea s_fliptank2,a1
	move frames,d7
	asl #2,d7
	add.b d7,d0
	move.l d5,-(a7)
...
clearfeed: move.l #(PITCH1|PIXEL16|WID384),d0
	move.l d0,source_flags
	move.l d0,dest_flags
	lea in_buf,a0
	move.l cscreen,(a0)		;source screen is already-displayed screen
...
versionscreen: move.l #rrts,routine
;	move #-1,db_on
	jsr InitBeasties
 	lea beasties,a0		;set main screen to 16-bit
	move.l #screen2,d2
...
dopyr:	move #64,d0			;glowing pyramid (selector?)
	move #80,d1	
	move #192,d6
	move #120,d7
dop:	add.l #$40000,pc_1
	add.l #$60100,pc_2
	bsr makepyr
...
optionscreen: move.l #rrts,routine
;	move #-1,db_on
	move.l #optiondraw,demo_routine
	clr.l pongx
	clr.l pongy
...
ssys:	move.b sysflags,d0
	and.l #$ff,d0
	move.l d0,_sysflags
	rts
optiondraw: move.l #1,gpu_mode
	bsr ssys
...
drawopts: bsr text_setup		;do generic text setup
...
g_textlength: move.l d2,a2	;d2.l points to 0term text
	move #0,d7
	move 6(a1),d6
	add #2,d6
...
inst:	move pongzv,d0			;get current CB pointer
...
m7run:	btst.b #3,sysflags		;are we on the rotary controller?
	beq wjoy
...
gatefx: dc.l rrts,zup,zdn,spup,vicend

zup: move.l #-$80000,iycon+4
	rts
zdn: move.l #$80000,iycon+4
	rts
spup: move.l d1,-(a7)
	add.l #5,yespitch
	eor #1,yesnum
...
agagg:	move.l (a7)+,d1
	rts
vicend: move.l #failfade,demo_routine
...
viccount: sub #1,pongx
	bpl rrrts
	move #1,x_end
	move #1,screen_ready
	rts

failcount: sub #1,pongx
	bpl rrrts
	move #-1,x_end
	move #1,screen_ready
	rts
...
setdsplay: jsr eepromsave
	...
	bsr showscores
	rts
getinitials: move keyplay,d4	;was player using a Key?
	bmi stditals		;no
okey:	lsl #2,d4
	lea keys,a0
	lea entxt,a1
...
setkey: move #2,d4
skey:	move.b (a1)+,(a0)+	;a1/a0 already set by FPLACE
	dbra d4,skey
	move.b t2k_max+1,d0
	sub.b #1,d0
	bclr #0,d0
...
fplace:	lea keys,a0		;Happy, happy, joy, joy.  You got an easy Key.
	lea entxt,a1
lkey0:	move #2,d4
	move.l a1,a2
	move.l a0,a3
...
getbrag: move #5,ennum
	move #2,fw_sphere
	move.l #conm4,enl1
	move.l #conm5,enl2
	move.l #conm6,enl3

txenter: move.l #rrts,routine
;	move #-1,db_on
	clr sync
	bsr wnb
;	bsr InitBeasties
...
hisettrue3: move #TOP-16,d1
	bra settrue33

showscores: move.l #rrts,routine
;	move #-1,db_on
	clr sync
	bsr InitBeasties
	bsr hisettrue
...
yh: 	move.l #4,gpu_mode	;Multiple images stretching towards you in Z
 	lea in_buf,a0
	move.l a2,(a0)+	;srce screen for effect
	move.l d0,(a0)+	;srce start pixel address
	move.l d1,(a0)+	;srce size
...
draw_oneup: move.l #4,gpu_mode	;Multiple images stretching towards you in Z
	move #0,d7
	move.l 12(a6),d6
	move #150,d5
...
draw_mpixex: move.l #pic,a2
giff: 	move.l #4,gpu_mode	;Multiple images stretching towards you in Z
	move #0,d7
	move.l 12(a6),d6
mpixex:	lea in_buf,a0
	move.l a2,(a0)+	;srce screen for effect
	move.l 36(a6),(a0)+	;srce start pixel address
	move.l 46(a6),(a0)+	;srce size
	move.l 42(a6),d0
...
gringbull: move.l #3,gpu_mode	;Multiple images stretching towards you in Z
	lea in_buf,a0
	move.l #pic2,(a0)+	;srce screen for effect
...
ringa2:	move.l #$0f000f,(a0)+	;srce size
	move.l #$8000,d7
...
ppolysi: move d0,d2
	...
	move d1,74(a0)	
	move #3,d4
ppsize:	move d2,16(a0)
	move d3,18(a0)
	lea 24(a0),a0
	dbra d4,ppsize
	rts

makepyr: movem d6-d7,-(a7)
	move pc_1,d7
	bsr pulser
	move d6,d4
	move d4,popo1
...
itpoly: move ranptr,rpcopy		;so sequence is always the same
rtpoly: move frames,d6
	lea sines,a0
	lea testpoly,a1
	move #2,d7
rtp:	jsr rannum
...
drang: sub.l #$2000,pongxv
	rts
irang: add.l #$2000,pongxv
	rts
ixpos: add #4,pongyv
	rts
dxpos: sub #4,pongyv
...
idelta: add.l #$4000,delta_i
	and.l #$ffffff,delta_i
	rts
ddelta: sub.l #$4000,delta_i
	and.l #$ffffff,delta_i
	rts
...
arts:	clr _pauen
	...
	move.l d0,selbutt
	rts	
thang:	bsr db				;sync with frame int, receive new drawscreen base, do user routine
	move.l demo_routine,a0
	jsr (a0)
	tst pawsed
	beq mooocow
...
gogame:  move #1,screen_ready		;as above, attract mode version
	clr db_on
	clr x_end
;	move pauen,_pauen
gog:	bsr thang
...
drxdec:	sub.l #$1,(a4)
	rts
dryinc:	add.l #$100,24(a4)
	rts
drydec:	sub.l #$100,24(a4)
	rts

drive2:	move.l #drxinc2,a0
	move.l #drxdec2,a1
	move.l #dryinc2,a2
	move.l #drydec2,a3
	bra gjoy

drxinc2: add.l #$1,4(a4)
	rts
drxdec2: sub.l #$1,4(a4)
	rts
dryinc2: add.l #$100,28(a4)
	rts
drydec2: sub.l #$100,28(a4)
	rts

drive3:	move.l #drxinc3,a2
	move.l #drxdec3,a3
	move.l #dryinc3,a0
	move.l #drydec3,a1
	bra gjoy

drxinc3: add.l #$1000,12(a4)
	rts
drxdec3: sub.l #$1000,12(a4)
	rts
dryinc3: add.l #$1000,16(a4)
	rts
drydec3: sub.l #$1000,16(a4)
	rts

ppong: move.l pongxv,d0
	add.l d0,pongx
	move.l pongyv,d0
	add.l d0,pongy
	bra pingzz
...
curcon: move.b pad_now+1,d0		;byte with UDLR in
	move cursx,d1
	move cursy,d2
	btst #4,d0		;test Up
	beq curc1
...
clsps:	clr.l (a0)+
	dbra d0,clsps
	bsr initobjects
rsetw:	move cwave,d0		;move to next wave
	move d0,d1
	clr.l d2
	tst t2k
...
swip1:	bsr sweb
	...
	move #7,d0
	clr d1
addbon:	move.b -(a1),d2		;bonus digit
...
swebcol: lea _web,a0		;set all the web to current colour
	move.l (a0),a1
...
swebpsych: lea _web,a0		;set all the web to psychedelic colours
	move.l (a0),a1
;	move.l #0,(a1)		;set web all rotate
	move frames,d7
	asl #1,d7
...
h2hclaws: move.l freeobjects,a6
	move.l a6,_claw
 	move.l a6,a0
	move.l #0,(a0)
	clr.l 4(A0)
...
make_bits: move.l #$00,d0			;parameters for object - XYZ scale...
	move.l #$01,d1
	move.l #$01,d2	
...
make_webs: lea raw_webs,a6
	lea webs,a5
;	move #15,d7
mk_webs: movem.l a5-a6/d7,-(a7)
	move.l (a6),a1
	move.l #40,a2
...
gsmart:	sub #1,szap_avail
	...
	move.l #$8000,d1
	bsr setmsg
nmsg:	move #1,szap_on			;Start the sizzle...
	move #7,sfx
	move #10,sfx_pri
	jsr fox
	rts	
...
reflshot: neg.l 36(a6)
	...
	jsr fox
	bra moomoomoo	
np2sh:	cmp #webz+81,12(a6)
	bmi rrts
kllit:	move.l 24(a6),a1
	clr.l (a1)		
	move #3,50(a6)
	rts
upweb: add.l d0,12(a6)
	move.l _claw,a4
	move.l 56(a4),a4	;p2's Claw
	move.l 56(a4),a4	;p1's Shield
	move 16(a6),d0
...
make_h2hball: move.l freeobjects,a0
	move web_max,d1
	jsr rand
	move #webz,d1
	swap d1
...
h2hballmodes: dc.l gstartmove,gmove,breset


breset:	clr 20(a6)
	rts

make_h2hgen: move.l freeobjects,a0
	move.l #-15,(a0)
	move d0,d2
	move #webz,d0
	swap d0
...
run_h2hgen: add #1,28(a6)
	...
	add #1,12(a6)
	rts
hupweb: cmp #webz-50,12(a6)
	bmi rrts
	sub #1,12(a6)
	rts

h2hgenmode: dc.l gstartmove,gmove,ggen

gstartmove: 	bsr webinfo			;get info on our lane
	move d4,d6
	move d5,d7			;preserve midpoint of our channel
gsmove:	tst 22(a6)
	bmi gleft	
...
run_ashot: add #4,28(a6)		;spin the shot
	add.b #$30,41(a6)		;make it flash
...
adenoid: move #$3c,bulland	;for test droid mode
	add #8,bullmax
	move.l a6,-(a7)
	bsr mkdroid
	move.l (a7)+,a6
...
run_zap: sub #1,42(a6)			;expand it
	add #10,28(a6)			;spin it too, why not
	cmp #-2,42(a6)
	bge rrts
kikik:	move #1,50(a6)			;tell FG to unlink
	rts


make_adroid: tst afree
	bmi rrts
	sub #1,afree
...
adroidmodes: dc.l admove,adgmove,zappage,uppweb
...
flipmodes: dc.l rail,flipto,stopped
...
stdrail: bsr collie			;Coll det
	bne fkillme			;I'm dead!
ssar:	bsr alienfire
	move.l flip_zspeed,d0
srail:	sub.l d0,12(a6)			;move Flipper vertically
	cmp #webz-80,12(a6)
	bgt rrts
	move #webz-80,12(a6)
sra:	clr 14(a6)
flipimm: move #1,24(a6)			;change flipper mode to Flip
	bsr flip_set
	rts
...
osnee:	move #1,d0
	rts
oclan2: 	cmp #webz-80,12(a0)	;check if on top (may be jumping!)
	blt clnxt
	clr d2
	rts
...
stopped: move 44(a6),d0
	lsl #2,d0
	lea sstopmodes,a0
	move.l 0(a0,d0.w),a0
	jmp (a0)
sstopmodes: dc.l stdstop,sustop1,sustop1,sustop1,sustop1,stdstop
...
gotu: 	cmp #-2,wave_tim		;-2 means the zoom has started and player is safe
...
tankercontents: dc.l openflippers,openfuseballs,openpulsars

openflippers: move.l a6,a5		;mummy's address save
	move.l freeobjects,a6
	bsr newflipper			;new flipper in this lane
	bsr flip_set_right
	move #0,d1
...
openpulsars: move.l a6,a5		;mummy's address save
	move.l freeobjects,a6
	bsr newflipper			;new flipper in this lane
	bsr flip_set_right
	move.l _pus+12,d0
...
spiker_vex: dc.l newspike,climbspike,descendspike
...
sfkillme: add #1,max_spikers
	bra fkillme

newspike: lea spikes,a0		;Pointer to table of existing spikes.
...
nxtspik: lea 4(a0),a0		;next spiketable entry
	add #1,d0
	cmp web_max,d0
	blt nuspik
...
nolonger: sub #1,36(a6)		;dec build duration
	bpl rrts
sdesc: 	move #2,44(a6)		;set to descend
	rts

descendspike: bsr alienfire
 	move.l spiker_zspeed,d0
	add.l d0,12(a6)
	cmp #webz+80,12(a6)
	blt rrts		;climb down
...
take_me_to_your_leader: move.l tmtyl,a0
	move.l _claw,a1
	jsr zoomdown
	add #2,12(a0)
	add #2,12(a1)
	cmp #webz+80,12(a1)
	blt rrts
setsnatch: move.l #snatch_it_away,routine	;Tuff titty
	clr l_soltarg
	rts
...
dxle:	move.l 24(a4),a1
	...
	move d0,46(a4)
	bra xle2
xle3:  cmp #6,54(a6)		;check for power ring hitting super spike
...
go_con: move 46(a6),d0		;skip on to clawcon (can move and fire during rezz)
	asl #2,d0
	lea run_vex,a0
	move.l 0(A0,d0.w),a0
	jmp (a0)
...
dshrnk:	move view,d0
	...
	rts
dpshift: move d2,vp_xtarg
	move d3,vp_ytarg		;Viewpoint targets
	rts

vp_set2: move.l _claw,a0
	move 4(a0),d2
	move 8(a0),d3
	move.l 56(a0),a0
	sub 4(a0),d2
...
rroco:	rol.b #2,d0			;Phase Bits to bottom of word
...
gamefx:	move.l vp_sfs,d0	;move the starfield's vp
	add.l d0,vp_sf
...
decco1: move #-1,modtimer
	rts

setmsg: move #100,msgtim1	;set default Messager parameters
	move #50,msgtim2
	move.l d0,msgxv
	move.l d1,msgyv
	move.l #$10000,msgxs
...
beastiesoff: neg beasties+76
	neg beasties+140
	rts


paustuff: jsr text2_setup
;	move.l #$400040,32(a0)
;	move.l #vertext,(a0) 
; 	lea texter,a0
;	jsr gpurun
...
run_vex: dc.l rrts,player_shot,run_flipper,run_zap,kill_shot,run_tanker,run_spike,run_spiker,run_ashot,run_fuseball,blowaway	;10
...
draw_vex: dc.l rrts,draw,draw_z,draw_vxc,draw_spike,draw_pixex,draw_mpixex,draw_oneup,draw_pel,changex	;9
...
apal:	move.l 60(a6),-(a7)		;go to next object
	...
	jsr gpuwait
nxt_o:	clr locked
nxt_ob:	move.l (a7)+,a6		;this way i can even trash a6 if i need II
...
noo_wb: move.l activeobjects,a6
...
; traverse the priority list, draw the objects on p1's web, then traverse it backwards to draw the objects on p2's web
	...
	move.l apriority,a0
 	cmp.l #-1,a0
	beq rrts		;End of list was reached
d2poloop: move.l (a0),a6		;Get object handle
	move.l (a6),d0
	move.l a0,-(a7)
	bsr podraw		;Go do object type draw
	jsr gpuwait
...
onlyball: lea in_buf+4,a0
	...
	move.l d0,32(a1)	;YZ orientation
	move #11,d0		;copy 48 bytes
bhead: 	move.l (a1)+,(a0)+	;copy the header to GPU input ram
	dbra d0,bhead
	move.l #$88,(a0)+		;colour
 	move.l #-2,scaler
	bra godraa
...
initobjects: move.l #-1,activeobjects
 lea objects,a0	
 move #63,d0
 move d0,ofree
...
initprior: move.l #-1,apriority
	lea priors,a0
	move #63,d0
	move.l a0,fpriority
	move.l #-1,a1
inipri:	move.l a1,4(a0)
	move.l a0,a1
	lea 16(a0),a0
	move.l a0,8(a1)
	dbra d0,inipri
...
setnewgen: lea wstuff,a1	;point at wave stuff table
	move #-1,(a1)+
...
thaggit: move d0,4(a1)		;-1 to terminate
	clr wave_tim
	rts
...
inits: dc.l make_flipper,make_tanker,make_spiker,make_fuseball,make_pulsar,make_futanker,make_putanker	;6
...
strin:	move.l wave_ptr,a6	;get current address in $
...
rst:	lea field1,a0
	...
	move #$0000,d4
ring1:	move #32,d6		;64 stars per ring
ring2:
;	bsr rannum
;	and #$0f,d0
...
xxnum:	lea 7(a2),a4	;place to expand the number into
...
zapreset: lea defaults,a0		;reset defaults
	lea hscom1,a1
	move #57,d0
ccrset:	move (a0)+,(a1)+		;copy var defaults to ram
	dbra d0,ccrset
	jsr spall			;(makesure PAL bit is set)
	bra eepromsave
...
g_getline: move.l a5,a2	;d2.l points to 0term text
	move #0,d7
...
g_gt1: 	cmp.b #'>',d5
	...
	add d5,d0
	bra g_gtlin
g_gt2:	move.b d5,(a3)+		;copy to line buffer
	add d6,d7
	move d0,d5
	add d7,d5
	cmp d3,d5
	bmi g_gtlin		;wait till a letter goes too far over
bakkup:	move.b #0,-(a3)		;back up till a space
	sub d6,d7
	move.b -(a2),d5
	cmp.b #' ',d5
	bne bakkup
...
azonk:	lea 20(a1),a1
	...
	cmp #3,d5
	blt xky
stetoff: move d5,akeys
	move.l #0,(a5)
	rts

playtune: jsr STOP_MOD
 	lsl #2,d0
	lea modbase,a0
...
waves:	dc.l _nw1,_nw2,_nw3,_nw4,_nw5,_nw6,_nw7,_nw8,_nw9,_nw10,_nw11,_nw12,_nw13,_nw14,_nw15,_nw16
...
w16col:	dc.w 96,64,32,0,48,48,48,0,-48,-48,-48,0,-32,-64,-96
	dc.w 0,$10,$20,$30,$40,$50,$60,$70,$60,$50,$40,$30,$20,$10,0
...
rantab:
	.include "rantab.dat"
solids: dc.l rrts,cdraw_sflipper,draw_sfliptank,s_shot,draw_sfuseball,draw_spulsar,draw_sfusetank,ringbull,draw_spulstank	;8
